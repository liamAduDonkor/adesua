...
const roleMenus: Record<string, { title: string; items: { title: string; href: string }[] }> = {
    student: {
        title: 'Student',
        items: [
...
export function AppHeader({ breadcrumbs = [] }: AppHeaderProps) {
    ...
    const roles: string[] = (auth?.roles as string[]) || [];
    const visibleRoleKeys = roles.length > 0 ? roles : Object.keys(roleMenus);
                        ...
                        <NavigationMenu className="flex h-full items-stretch">
                            <NavigationMenuList className="flex h-full items-stretch space-x-2">
                                ...
                                {visibleRoleKeys.map((key) => {
                                    const role = key as keyof typeof roleMenus;
                                    const cfg = roleMenus[role];
                                    if (!cfg) return null;
                                    
                                    return (
                                        <NavigationMenuItem key={`role-${role}`} className="relative flex h-full items-center">
                                            ...
                                            <NavigationMenuContent>
                                                <div className="grid w-[400px] gap-3 p-4">
                                                    {cfg.items.map((item) => (
                                                        <NavigationMenuLink asChild key={`${role}-${item.title}`}>
                                                            <Link
                                                                href={item.href}
